 ...
 */
final case class KafkaKeyAvroMessageEnvelope(
    serviceName: String,
    key: String,
  ...
  lazy val messageBytes: Array[Byte] =
    ...
    new GenericDatumWriter[GenericRecord](avroValue.getSchema).write(avroValue, encoder)
    encoder.flush()
    out.toByteArray
...
object KafkaKeyAvroMessageEnvelope:
  def apply(
      serviceName: String,
      eventsourcedEnvelope: EventSourcedEventEnvelope[?],
      avroValue: GenericRecord
  ): KafkaKeyAvroMessageEnvelope =
    new KafkaKeyAvroMessageEnvelope(
      serviceName,
  ...
  def apply(
      serviceName: String,
      persistenceQueryEnvelope: PersistenceQueryEventEnvelope[?],
      avroValue: GenericRecord
  ): KafkaKeyAvroMessageEnvelope =
    new KafkaKeyAvroMessageEnvelope(
      serviceName,
